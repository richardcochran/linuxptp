...
* Features
  ...
  - Supports IEEE 802.1AS-2011 and 802.1AS-2020 in the role of end
    station. 802.1AS-2020 support includes multi-time domain operation
    with Common Mean Link Delay Service (CMLDS).
  ...
  - Supports a number of profiles, including:
    ...
    - The IEEE 802.1AS/gPTP profiles, with and without CMLDS support.
...
** Optional DPDK Transport Build

  The project can be built with an optional DPDK transport backend.
  When enabled, select it at runtime using the configuration option
  ~network_transport DPDK~.

  Build with DPDK support:

#+BEGIN_EXAMPLE
  PKG_CONFIG_PATH=/path/to/dpdk/lib64/pkgconfig:/path/to/dpdk/lib/pkgconfig \
  make DPDK=1
#+END_EXAMPLE

  If ~pkg-config~ can resolve ~libdpdk~, the makefile adds the
  required compile/link flags and embeds the discovered DPDK
  library directory as an rpath, so running binaries do not require
  setting ~LD_LIBRARY_PATH~.

  If ~libdpdk~ metadata is not available, the build continues with
  DPDK glue stubs and prints a warning. In this mode, the DPDK
  transport backend is not fully functional until valid DPDK flags
  are provided.
